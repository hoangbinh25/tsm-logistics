...
export default function DriverNotificationsPage() {
    ...
    useEffect(() => {
        const fetchNotis = async () => {
            try {
                const res = await fetchWithAuth(`${process.env.NEXT_PUBLIC_API_URL}/notifications`)
                ...
                <div className="divide-y">
                    {loading ? (
                        ...
                        notifications.map((notif) => (
                            ...
                            >
                                {/* Icon */}
                                <div className={`w-10 h-10 rounded-full shrink-0 flex items-center justify-center ${notif.type === 'ORDER' ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-600'
                                    }`}>
